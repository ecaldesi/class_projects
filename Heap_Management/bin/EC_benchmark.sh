...
test-library() {
    ...
    echo "Results: $library..." >> ./EC_benchmark_results.txt
	for index in {1..9}
	do    
		second=$(($index+9))
		third=$(($index+18))
	    ...
	    echo "$index :: $tmpVar5" >> ./EC_benchmark_results.txt
	done
...
rm ./EC_benchmark_results.txt
echo "-------------------------------------------------------------------------------------------------------------------------" >> ./EC_benchmark_results.txt
echo "1 - Mallocs | 2 - Frees | 3 - Reuses | 4 - Grows | 5 - Splits | 6 - Coalesces | 7 - Blocks | 8 - Requested | 9 - Max Heap" >> ./EC_benchmark_results.txt
echo "-------------------------------------------------------------------------------------------------------------------------" >> ./EC_benchmark_results.txt
test-library EC_libmalloc-ff.so
test-library EC_libmalloc-nf.so
test-library EC_libmalloc-bf.so
test-library EC_libmalloc-wf.so
